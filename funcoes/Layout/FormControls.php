...
class FormControls
{
    ...
    public static function helpText($text)
    {
        if ($text == '') {
            return '';
        }
    ...
    public static function input($label, $name, $value = "", $attrs = [])
    {
        ...
        $help = self::helpText($attrs['help'] ?? '');
        $has_label = $attrs['has_label'] ?? true;
        $add_to_div = $attrs['add_to_div'] ?? '';
        $after = $attrs['after'] ?? '';
        ...
        $html = self::inputHTML($name, $value, $attrs);
        return <<<HTML
            <div class="form-group $div_class" $add_to_div>
                $label_for
                $html
                $help
                $after
    ...
    public static function inputHTML($name, $value = "", $attrs = [])
    {
        $id = $attrs['id'] ?? $name;
