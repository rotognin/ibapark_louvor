...
class Usuario extends DAO
{
    ...
    private function baseQuery($where)
    {
        ...
        $sql = "SELECT 
            ...
            e.emp_nome
        FROM {$this->table('usuario')} u
        LEFT JOIN {$this->table('usuario_empresa')} ue 
            ON ue.usu_login = u.usu_login AND  ue.emp_padrao = 'S'
        LEFT JOIN {$this->table('empresa')} e 
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('usuario'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    {
        [$sql, $args] = $this->preparedUpdate($this->table('usuario'), $record);
        $sql .= " WHERE usu_login = ?";
        $args[] = $usu_login;
    ...
    public function getEmpresas($usu_login)
    {
        $sql = "SELECT 
            ...
            e.emp_nome,
            ue.emp_padrao
        FROM {$this->table('empresa')} e
        INNER JOIN {$this->table('usuario_empresa')} ue 
            ON e.emp_codigo = ue.emp_codigo
        WHERE ue.usu_login = ?
    ...
    {
        $sql = "UPDATE {$this->table('usuario_empresa')} 
        SET emp_padrao = 'N'
        WHERE usu_login = ?
        ";
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('usuario_empresa'), [
            'usu_login' => $usu_login,
    ...
    {
        $sql = "DELETE FROM {$this->table('usuario_empresa')} 
    ...
    public function getAcoes($usu_login): array
    {
        $sql = "SELECT 
            ...
            a.aca_descricao,
            a.aca_grupo
        FROM {$this->table('acao')} a
        INNER JOIN {$this->table('usuario_acao')} ua 
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('usuario_acao'), [
            'usu_login' => $usu_login,
            'aca_acao' => $aca_acao
        ]);
    ...
    {
        $sql = "DELETE FROM {$this->table('usuario_acao')} 
    ...
    public function copiarAcoes($src, $dst)
    {
        ...
        $sql = "INSERT INTO {$this->table('usuario_acao')} (usu_login, aca_acao)
        SELECT ?, aca_acao FROM {$this->table('usuario_acao')} WHERE usu_login = ?
        ";
