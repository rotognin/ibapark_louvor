...
class Menu extends DAO
{
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('programa'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    public function update(string $prg_codigo, array $record): int
    {
        ...
        [$sql, $args] = $this->preparedUpdate($this->table('programa'), $record);
    ...
    {
        $sql = "DELETE FROM {$this->table('programa')} WHERE prg_codigo = ?";
        $stmt = $this->default->prepare($sql);
