...
class LogPrograma extends DAO
{
    ...
    public function getMaisAcessados($where, $limit = 5)
    {
        $sql = "SELECT p.prg_descricao, l.prg_codigo, COUNT(l.prg_codigo) as Acessos, p.prg_url, p.prg_icone, p.prg_codigo_pai 
            FROM {$this->table('log_programa')} l
            LEFT JOIN {$this->table('programa')} p ON l.prg_codigo = p.prg_codigo
            WHERE 1=1 ";
    ...
    public function total($where)
    {
        $sql = "SELECT COUNT(log_codigo) AS total 
                FROM {$this->table('log_programa')} l
                INNER JOIN {$this->table('usuario')} u ON (l.usu_login = u.usu_login)
                INNER JOIN {$this->table('programa')} p ON (l.prg_codigo = p.prg_codigo)
                WHERE 1=1 ";
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('log_programa'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
