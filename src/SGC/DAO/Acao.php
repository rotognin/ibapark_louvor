...
class Acao extends DAO
{
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('acao'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    {
        [$sql, $args] = $this->preparedUpdate($this->table('acao'), $record);
        $sql .= " WHERE aca_acao = ?";
        $args[] = $aca_acao;
    ...
    {
        $query = "
            SELECT distinct aca_grupo FROM {$this->table('acao')} ORDER BY aca_grupo
        ";
    ...
    {
        $sql = "DELETE FROM {$this->table('acao')} WHERE aca_acao = ?";
        $stmt = $this->default->prepare($sql);
        $stmt->execute([$aca_acao]);

        $sql = "DELETE FROM {$this->table('usuario_acao')} WHERE aca_acao = ?";
        $stmt = $this->default->prepare($sql);
        $stmt->execute([$aca_acao]);
        return $stmt->rowCount();
    }
