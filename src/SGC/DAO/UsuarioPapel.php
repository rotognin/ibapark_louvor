...
class UsuarioPapel extends DAO
{
    ...
    public function getArrayCompleta($where = [], $order = null, $limit = null, $offset = '0'): array
    {
        ...
        $sql = "SELECT 
            ...
            u.usu_nome,
            p.pap_descricao
            FROM {$this->table('usuario_papel')} x
            LEFT JOIN {$this->table('usuario')} u ON x.usupap_usu_login = u.usu_login
            LEFT JOIN {$this->table('papel')} p ON x.usupap_papel_id = p.pap_id
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('usuario_papel'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    {
        [$sql, $args] = $this->preparedUpdate($this->table('usuario_papel'), $record);
        $sql .= " WHERE usupap_id = ?";
        $args[] = $usupap_id;
