...
class AssinaturasPasso extends DAO
{
    ...
    public function getArray($where = [], $order = null, $limit = null, $offset = '0'): array
    {
        ...
        $sql = "SELECT 
            {$campos} 
            FROM {$this->table('assinaturas_passo')} 
            WHERE 1=1 
        ";
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('assinaturas_passo'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    {
        [$sql, $args] = $this->preparedUpdate($this->table('assinaturas_passo'), $record);
        $sql .= " WHERE asspas_id = ?";
        $args[] = $asspas_id;
