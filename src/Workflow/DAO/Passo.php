...
class Passo extends DAO
{
    ...
    public function getArray($where = [], $order = null, $limit = null, $offset = '0'): array
    {
        ...
        $sql = "SELECT 
            {$campos} 
            FROM {$this->table('workflow_passos')} 
            WHERE 1=1 
        ";
    ...
    {
        [$sql, $args] = $this->preparedInsert($this->table('workflow_passos'), $record);
        $stmt = $this->default->prepare($sql);
        $stmt->execute($args);
    ...
    {
        [$sql, $args] = $this->preparedUpdate($this->table('workflow_passos'), $record);
        $sql .= " WHERE pas_id = ?";
        $args[] = $pas_id;
