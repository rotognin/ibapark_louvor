...
// Checar se temcookie de login
if ($posicao == 'form') {
    $cookie_login = $request->getCookie('sistema_rotog_2025');
    if ($cookie_login != '') {
        $dao = new App\SGC\DAO\Usuario();
        $empresaDAO = new App\SGC\DAO\Empresa();

        $where = array('');
        $where[0] = ' AND u.usu_login = ?';
        $where[1][] = $cookie_login;

        $aUsuario = $dao->getArray($where);
...
if ($posicao == 'form') {
    ...
    // Cookie após logar
    $cookie_name = 'sistema_rotog_2025';
    $cookie_value = $usuario['usu_login'];
    setcookie($cookie_name, $cookie_value, time() + 60 * 60 * 25 * 30); // 30 dias
